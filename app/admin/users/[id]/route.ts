...
export async function POST(request: Request, context: RouteContext) {
  try {
    ...
    if (!['demote_to_free', 'grant_pro'].includes(body.action ?? '')) return NextResponse.json({ error: 'Invalid admin action.' }, { status: 400 });
    ...
    try {
      ...
      const userResult = await client.query<{ id: string; email: string; plan: string; subscription_status: string }>('select id, email, plan, subscription_status from users where id = $1 limit 1', [id]);
      ...
      if (body.action === 'grant_pro') {
        if (user.plan !== 'free') { await client.query('ROLLBACK'); return NextResponse.json({ error: 'Account is already Pro.' }, { status: 400 }); }
        await client.query(`update users set plan = 'pro', subscription_status = 'lifetime', stripe_subscription_id = null, subscription_current_period_end = null where id = $1`, [id]);
        await client.query('COMMIT');
        await recordAdminAction(admin.email, 'grant_pro_free', 'user', id, { previousPlan: user.plan, previousSubscriptionStatus: user.subscription_status, accessType: 'lifetime' });
        let emailSent = true;
        try { await sendLifetimeProEmail(user.email); } catch (emailError) { emailSent = false; console.error('TripSignal lifetime Pro email failed:', emailError); }
        return NextResponse.json({ ok: true, plan: 'pro', subscriptionStatus: 'lifetime', emailSent });
      }

      if (user.plan === 'free') { await client.query('ROLLBACK'); return NextResponse.json({ error: 'Account is already Free.' }, { status: 400 }); }
      if (['active', 'trialing'].includes(user.subscription_status)) { await client.query('ROLLBACK'); return NextResponse.json({ error: 'This account has an active or trialing subscription and cannot be demoted by this action.' }, { status: 400 }); }
      const alertsResult = await client.query<{ id: string }>(`select id from alerts where user_id = $1 or lower(email) = lower($2) order by created_at asc, id asc`, [id, user.email]);
      await client.query(`update users set plan = 'free', subscription_status = 'canceled', stripe_subscription_id = null, subscription_current_period_end = null where id = $1`, [id]);
      const extraAlertIds = alertsResult.rows.slice(1).map((row) => row.id);
      if (extraAlertIds.length) await client.query('update alerts set active = false where id = any($1)', [extraAlertIds]);
      await client.query('COMMIT');
      await recordAdminAction(admin.email, 'demote_user_to_free', 'user', id, { previousPlan: user.plan, previousSubscriptionStatus: user.subscription_status, pausedAlerts: extraAlertIds.length });
