...
export default function AlertBuilder() {
  ...
  const summary = useMemo(
    () => `${origin || 'MCI'} → ${destinationLabel} · ${cabin === 'premium_economy' ? 'Premium economy' : cabin === 'business' ? 'Business' : 'Economy'} · under $${Number(price || 0).toLocaleString()} · ${frequency}`,
    [origin, destinationLabel, cabin, price, frequency],
  );
        ...
        <form className="builder-grid" onSubmit={submit}>
          <div className="builder-form">
            <section className="form-section">
              <div className="form-heading"><span>01</span><div><h2>Where are you going?</h2><p>Choose a broad region or enter an exact airport.</p></div></div>
              ...
              <fieldset>
                ...
                <div className="select-row">
                  <button type="button" className={destinationMode === 'region' ? 'option active' : 'option'} onClick={() => setDestinationMode('region')}>General location</button>
                  <button type="button" className={destinationMode === 'airport' ? 'option active' : 'option'} onClick={() => setDestinationMode('airport')}>Specific airport</button>
                </div>

                {destinationMode === 'region' ? (
                  <label className="nested-field"><span>Region</span><select value={region} onChange={(event) => setRegion(event.target.value)}>{regions.map((item) => <option key={item}>{item}</option>)}</select><small>TripSignal searches a defined set of airports within the selected region.</small></label>
                ) : (
                  <label className="nested-field"><span>Airport code</span><input value={destinationAirport} onChange={(event) => setDestinationAirport(event.target.value.toUpperCase())} placeholder="AMS" maxLength={3} /><small>Enter the exact three-letter airport you want to monitor.</small></label>
                )}
              </fieldset>
            </section>
