...
export default function EditAlert({ id }: { id: string }) {
  ...
  async function save(event: FormEvent) {
    ...
    try {
      ...
      const d = await r.json(); if (!r.ok) throw new Error(d.error || 'Could not save changes.');
  ...
  const oneWay = criteria.tripType === 'one-way';
    ...
    <section className="builder shell">
      ...
      <form className="builder-grid" onSubmit={save}><div className="builder-form">
        <section className="form-section"><div className="form-heading"><div><h2>Trip</h2><p>Change the route or destination.</p></div></div>
          ...
          <label><span>Destination type</span><select value={criteria.destinationMode || 'airport'} onChange={e=>update('destinationMode',e.target.value)}><option value="airport">Specific airport</option><option value="country">Country</option><option value="region">General location</option></select></label>
          {criteria.destinationMode === 'region' ? <label><span>Region</span><select value={criteria.destination || 'Europe'} onChange={e=>update('destination',e.target.value)}>{regions.map(r=><option key={r}>{r}</option>)}</select></label> : criteria.destinationMode === 'country' ? <label><span>Destination country</span><input value={criteria.destination || ''} onChange={e=>update('destination',e.target.value.toUpperCase())} maxLength={2} required /><small>Use the two-letter country code, such as HU for Hungary.</small></label> : <label><span>Destination airport</span><input value={criteria.destination || ''} onChange={e=>update('destination',e.target.value.toUpperCase())} maxLength={3} required /><small>Use the three-letter airport code.</small></label>}
          <fieldset><legend>Flight type</legend><div className="select-row"><button type="button" className={`option ${!oneWay?'active':''}`} onClick={()=>update('tripType','round-trip')}>Round trip</button><button type="button" className={`option ${oneWay?'active':''}`} onClick={()=>update('tripType','one-way')}>One way</button></div></fieldset>
        </section>
        <section className="form-section"><div className="form-heading"><div><h2>Fare</h2><p>Change what counts as a qualifying fare.</p></div></div>
          <label><span>Maximum {oneWay ? 'one-way' : 'round-trip'} price</span><div className="input-prefix"><b>$</b><input inputMode="numeric" value={criteria.maxPrice ?? ''} onChange={e=>update('maxPrice',Number(e.target.value.replace(/[^0-9]/g,'')))} required /></div></label>
          ...
          <div className="two-col"><label><span>Maximum stops</span><select value={criteria.maxStops || '1'} onChange={e=>update('maxStops',e.target.value)}><option value="0">Nonstop</option><option value="1">1 stop</option><option value="2">2 stops</option><option value="any">Any</option></select></label>{!oneWay && <label><span>Trip length</span><select value={criteria.tripLength || '1–3 weeks'} onChange={e=>update('tripLength',e.target.value)}><option>3–7 days</option><option>1–2 weeks</option><option>1–3 weeks</option><option>1–4 weeks</option></select></label>}</div>
        </section>
