...
export default function ResetPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.error || 'We could not send the reset link.');
      setSent(true);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'We could not send the reset link.');
    } finally {
      setLoading(false);
    }
  }
    ...
    <main className="auth-page">
      ...
      <section className="auth-card shell">
        <div className="auth-panel">
          ...
          {sent ? (
            <>
              <p>Check your inbox. If an account exists for <strong>{email}</strong>, we just sent it a link to set a new password. The link expires in 15 minutes.</p>
              <p className="auth-switch"><a href="/signin">Back to sign in</a></p>
            </>
          ) : (
            <>
              <p>Enter the email address for your TripSignal account and we&rsquo;ll send you a link to set a new password.</p>
              <form onSubmit={submit} className="auth-form">
                ...
                <button className="button button-primary auth-submit" type="submit" disabled={loading}>{loading ? 'Sending…' : 'Send reset link'}</button>
              </form>
              <p className="auth-switch"><a href="/signin">Back to sign in</a></p>
            </>
          )}
