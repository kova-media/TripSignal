...
function nextDepartureDate(alert: AlertCriteria, salt = 0) {
  ...
  const [, maxTripDays] = tripDays(alert.tripLength);
  const spanDays = Math.max(0, Math.round((end.getTime() - start.getTime()) / 86_400_000));
  const latestDepartureOffset = alert.tripType === 'one-way' ? spanDays : Math.max(0, spanDays - maxTripDays);
...
function buildCriteria(alert: AlertCriteria, salt = 0): FlightSearchCriteria {
  ...
  return {
    ...
    tripType: alert.tripType === 'one-way' ? 'one-way' : 'round-trip',
...
export function summarizeAlert(alert: AlertCriteria) {
  const destination = alert.destinationMode === 'airport' ? alert.destination.toUpperCase() : alert.destinationMode === 'country' ? alert.destination.toUpperCase() : alert.destination;
  ...
  const tripType = alert.tripType === 'one-way' ? 'One way' : 'Round trip';
  return `${alert.origin} → ${destination} · ${tripType} · ${cabin} · under $${alert.maxPrice.toLocaleString()} · ${alert.dateRange} · ${alert.frequency}`;
}

export async function runAlertSearch(alertId: string, email: string, criteria: AlertCriteria) {
  const db = getDb();
  const runInserted = await db.query<{ id: string }>('insert into alert_runs (alert_id, status) values ($1, $2) returning id', [alertId, 'running']);
  ...
  try {
    ...
    const qualifying = results.filter((offer) => offer.price < criteria.maxPrice).sort((a, b) => a.price - b.price).filter((offer, index, array) => index === array.findIndex((candidate) => candidate.id === offer.id)).slice(0, 10);
    ...
    for (const offer of newOffers) await db.query('insert into signals (alert_id, offer_id, offer) values ($1, $2, $3::jsonb) on conflict (alert_id, offer_id) do nothing', [alertId, offer.id, JSON.stringify(offer)]);
    ...
    if (runId) await db.query('update alert_runs set status = $1, finished_at = now(), error_message = $2 where id = $3', ['error', message.slice(0, 2000), runId]).catch(() => undefined);
    throw error;
  }
}
