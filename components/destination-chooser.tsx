...
type DestinationChooserProps = { onModeChange?: (mode: Mode) => void };
...
export default function DestinationChooser({ onModeChange }: DestinationChooserProps) {
  ...
  useEffect(() => {
    ...
    if (mode !== 'airport' || query.length < 2 || airport?.municipality === query) { setAirportResults([]); return; }
  ...
  useEffect(() => {
    ...
    if (mode !== 'country' || query.length < 2 || country?.name === query) { setCountryResults([]); return; }
    const controller = new AbortController();
    const timer = window.setTimeout(async () => {
      try {
  ...
  const portal = (
    <div className="destination-chooser-root">
      <div className="discovery-options">
        <button type="button" data-destination-mode="airport" className={mode === 'airport' ? 'active' : ''} onClick={() => { setMode('airport'); setOpen(false); }}>Specific airport</button>
        <button type="button" data-destination-mode="country" className={mode === 'country' ? 'active' : ''} onClick={() => { setMode('country'); setOpen(false); }}>Country</button>
      </div>
      {mode === 'airport' ? (
        <div className="discovery-airport-search destination-chooser-search">
          ...
          {open && airportResults.length > 0 && <div className="discovery-airport-results" role="listbox">{airportResults.map((result) => <button type="button" className="discovery-airport-result" key={`${result.iata_code}-${result.name}`} onMouseDown={(event) => event.preventDefault()} onClick={() => { setAirport(result); setAirportQuery(`${result.municipality || result.name} (${result.iata_code})`); setOpen(false); window.dispatchEvent(new CustomEvent<DestinationSelection>('tripsignal:destination-change', { detail: { mode: 'airport', airport: result } })); }}><strong>{result.municipality || result.name}</strong><span>{result.iata_code} · {result.name}</span></button>)}</div>}
        </div>
      ) : (
        <div className="discovery-airport-search destination-chooser-search">
          ...
          {open && countryResults.length > 0 && <div className="discovery-airport-results" role="listbox">{countryResults.map((result) => <button type="button" className="discovery-airport-result" key={result.code} onMouseDown={(event) => event.preventDefault()} onClick={() => { setCountry(result); setCountryQuery(result.name); setOpen(false); }}><strong>{result.name}</strong><span>{result.code} · Search across the country</span></button>)}</div>}
        </div>
      )}
    </div>
  );
