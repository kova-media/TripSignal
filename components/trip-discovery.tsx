  ...
  return (
    <section className="discovery shell" id="explore">
      <div className="discovery-head">
        <div><p className="section-kicker">Find a trip</p><h2>Tell us where.<br /><em>We'll watch the fare.</em></h2></div>
      ...
      <div className="discovery-layout">
        <div className="discovery-controls">
          ...
          {destinationMode === 'region' ? <div className="discovery-field"><label htmlFor="discovery-region">Region</label><select id="discovery-region" value={region} onChange={(event) => setRegion(event.target.value)}>{regions.map((item) => <option key={item}>{item}</option>)}</select></div> : <AirportSearch label="Destination" value={destinationSearch} code={destinationAirport} onSelect={(airport) => { setDestinationAirport(airport.iata_code); setDestinationSearch(`${airport.municipality || airport.name} (${airport.iata_code})`); setDestinationCoordinate(airport.latitude != null && airport.longitude != null ? [airport.longitude, airport.latitude] : undefined); }} placeholder="Search a city or airport" />}
          <div className="discovery-field"><label htmlFor="discovery-window">When</label><select id="discovery-window" value={dateRange} onChange={(event) => setDateRange(event.target.value)}><option>Anytime</option><option>Next 3 months</option><option>Next 6 months</option><option>Next 12 months</option></select></div>
          <div className="discovery-field"><div className="fare-target-head"><label htmlFor="discovery-budget">Fare target</label><strong>${budgetValue.toLocaleString()}</strong></div><div className="fare-target-control"><span aria-hidden="true">$</span><input id="discovery-budget" className="fare-target-input" type="number" min="300" max={cabinMax[cabin]} step="50" value={budget} onChange={(event) => setBudget(event.target.value)} onBlur={() => { const next = Math.min(Math.max(Number(budget) || 300, 300), cabinMax[cabin]); setBudget(String(next)); }} aria-label="Maximum fare target" /></div><div className="fare-target-labels"><span>Minimum $300</span><span>Up to ${cabinMax[cabin].toLocaleString()}</span></div></div>
          <div className="discovery-advanced-fields">
            <div className="discovery-field"><label>Cabin</label><div className="discovery-options discovery-cabin-options">{cabinOptions.map((item) => <button type="button" key={item} className={cabin === item ? 'active' : ''} onClick={() => setCabin(item)}>{item}</button>)}</div></div>
            ...
            <div className="discovery-field"><label htmlFor="discovery-stops">Maximum stops</label><select id="discovery-stops" value={stops} onChange={(event) => setStops(event.target.value)}><option value="0">Nonstop</option><option value="1">1 stop</option><option value="2">2 stops</option><option value="any">Any</option></select></div>
            ...
            <div className="discovery-field"><label htmlFor="discovery-frequency">Search frequency</label><select id="discovery-frequency" value={frequency} onChange={(event) => setFrequency(event.target.value as Frequency)}><option>Daily</option><option>Weekly</option><option>Monthly</option></select></div>
          ...
          <div className="discovery-field"><label htmlFor="discovery-email">Alert email</label><input id="discovery-email" type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="you@example.com" autoComplete="email" /></div>
